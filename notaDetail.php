...
if(!empty($_GET)){
    $dados = listarNota($_GET['i']);
    $tipoAvaliacao = listarTipoAvaliacao($dados->tipo_avaliacao_id);
    $md = listarMatriculaDisciplina($dados->matricula_disciplia_id);
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    <div class="fg-line form-group">
                                        <label>Valor</label>
                                        <p><?php echo moeda($dados->valor) ?></p>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="fg-line form-group">
                                        <label>Data da avaliacao</label>
                                        <p><?php echo formata_padrao_data($dados->data_avaliacao) ?></p>
                                    </div>
                                </div>
                                ...
                                <div class="col-sm-6">
                                    <div class="fg-line form-group">
                                        ...
                                        <p><?php echo $tipoAvaliacao->descricao ?></p>
                                    </div>
                                </div>                                
                            </div>
                        </div>
        ...
        <div class="modal fade" id="edit" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <form class="formularioEdit" action="services/nota.php" method="POST">
                        <input type="hidden" name="function" value="edit"/> 
                        <input type="hidden" name="id" value="<?php echo $dados->id ?>"/>
                        <input type="hidden" name="md" value="<?php echo $dados->matricula_disciplia_id ?>"/>
                            ...
                            <div class="card-body card-padding">
                                <div class="row">
                                    <div class="col-sm-6">
                                        <div class="fg-line form-group">
                                            <label>Valor</label>
                                            <input id="valor" type="text" value="<?php echo moeda($dados->valor) ?>" class="form-control input-sm" name="valor" required="">
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="fg-line form-group">
                                            <label>Data da avaliação</label>
                                            <input type="text" value="<?php echo formata_padrao_data($dados->data_avaliacao) ?>" class="form-control input-sm dataAvaliacao" data-toggle="datepicker" name="dataAvaliacao" required="">
                                        </div>
                                    </div> 
                                    <div class="col-sm-6">  
                                        <div class="fg-line form-group">
                                            <label>Tipo de avaliação</label><br>
                                            <select class="chosen" placeholder="Selecione" name="tipoAvaliacao">

                                                    <?php
                                                    if($tiposAvaliacao != "nenhum"){
                                                    foreach ($tiposAvaliacao as $tipo) {
//                                                        $nivel_select = listarNivel($curso->id_nivel);
                                                        ?>
                                                <option <?php if($dados->tipo_avaliacao_id == $tipo->id){?> selected="" <?php }?> value="<?php echo $tipo->id ?>"><?php echo $tipo->descricao ?></option>
                                                        <?php
                                                    }
                                                    }else{
                                                        echo 'nenhum tipo';
            ...
            $.ajax({
                    ...
                    success: function(data){
                        swal({
                            ...
                            location.href='notasDisciplina?m=<?php echo $md->id_matricula ?>&d=<?php echo $md->disc_id ?>';
                        });                         
